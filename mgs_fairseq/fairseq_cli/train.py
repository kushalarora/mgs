...
def train(args, trainer, task, epoch_itr, max_update=math.inf):
    ...
    if args.initial_validation or args.validate:
    ...
    for samples in progress:
        ...
        if num_updates % args.log_interval == 0:
            ...
            log_stats = {
                ...
                if k not in ['nll_loss', 'ups', 'wpb', 'train_wall', 'wall']
            }

            if hasattr(task, 'custom_metrics'):
                custom_metrics = task.custom_metrics['train'].normalize()
                for k, v in custom_metrics.items():
                    if 'distance' in k or 'highest' in k:
                        log_stats[k] = v

            progress.log(log_stats, tag='train_inner', step=num_updates)
    ...
    if hasattr(task, 'custom_metrics'):
        custom_metrics = task.custom_metrics['train'].normalize()
        task.custom_metrics['train'].reset()
        for k, v in custom_metrics.items():
            stats[k] = v
...
def cli_main(modify_parser=None):
    import fairseq.ggs.translation_ggs
