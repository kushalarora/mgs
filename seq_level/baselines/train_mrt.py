...
def get_mrt_args():
    parser = argparse.ArgumentParser(description='Minimum risk training.')
    parser.add_argument(
    ...
    parser.add_argument(
        '--mrt_mle_mix', type=float, default=0.1
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--offset_zero', type=int, choices=[0, 1], default=0
    )
    parser.add_argument(
...
def get_mrt_loss(model, candidates, distances, batch_size, eos_idx, args):
    ...
    # Ensure zero-distance candidates to receive non-zero gradients
    if args.offset_zero:
        distances += 1

    scores = (log_probs * seq_masks).sum(2) / seq_masks.sum(2)
    probs = torch.softmax(scores.exp(), 1)
    loss = (distances * probs).mean(1).sum()
...
def main():
    ...
    logger, exp_path = setup('MRT_{}_{}_{}_{}'.format(args.metric,
                                                      args.include_target,
                                                      args.normalized_distance,
                                                      args.mrt_mle_mix), args)
