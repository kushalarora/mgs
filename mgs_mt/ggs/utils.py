...
def task_distance(preds_trim, targets_trim, kind='edit', eos_id=None, bleu_smoothing='method4'):
    if kind == 'edit':
        ...
        scores = []
        for pred, target in zip(preds_trim, targets_trim):
            score = single_meteor_score(target, pred)
            scores.append(score)
...
def perturb(
    step, model, model_with_grad, noise, no_mle, keep_grad, keep_zero,
    noise_scaling='constant',
    last_layer_only=False,
):
    eps_eps = 0
    ...
    for param, (name, param_with_grad) in zip(model.parameters(),
                                                model_with_grad.named_parameters()):
        if last_layer_only:
            if 'decoder.layers.5' not in name:
                continue
